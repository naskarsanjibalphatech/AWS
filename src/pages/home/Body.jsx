...
const PLCDashboard = () => {
  ...
  const API_BASE = 'https://ug9f8ssgra.execute-api.ap-south-1.amazonaws.com/default';

  // Trigger the PLC / cloud update
  const triggerDataUpdate = async () => {
    try {
      await fetch(`${API_BASE}/CloudToUSR_GIC_USR`, {
        method: 'PUT',
  ...
  // Fetch energy metrics (Voltage, Current, KWH)
  const fetchEnergyValue = useCallback(async () => {
    try {
      await triggerEnergyMeterRead();

      // Addresses for Voltage (20001), KWH (20005), Current (20003)
      const response = await fetch(`${API_BASE}/CloudToUSR_GIC_USR?address=20001,20005,20003&last=1`);
      const data = await response.json();

      const getValue = (addr) => {
        const found = data.find(d => d.address === addr);
        return found ? found.value : null;
      };

      setEnergyValue(getValue(20001));
      setTotalKwh(getValue(20005));
      setCurrentValue(getValue(20003));
  ...
  const fetchOutputStatuses = useCallback(async () => {
    try {
      ...
      const addresses = [...Array(8).keys()];
      const response = await fetch(`${API_BASE}/CloudToUSR_GIC_USR?address=${addresses.join(',')}&last=1`);
      const data = await response.json();

      const statuses = addresses.map(i => {
        const found = data.find(d => d.address === i);
        return found ? found.value : null;
      });
      const timestamps = addresses.map(i => {
        const found = data.find(d => d.address === i);
  ...
  // Fetch Input Statuses
  const fetchInputStatuses = useCallback(async () => {
    try {
      const addresses = [...Array(8).keys()].map(i => 10000 + i);
      const response = await fetch(`${API_BASE}/CloudToUSR_GIC_USR?address=${addresses.join(',')}&last=1`);
      const data = await response.json();

      const statuses = addresses.map(addr => {
        const found = data.find(d => d.address === addr);
        return found ? found.value : null;
      });
  ...
  // Send push button command (Start/Stop)
  const sendPushButtonCommand = async (onAddress, offAddress, coilIndex, isStart) => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/?address=${preClearAddress}&value=0`, { method: 'POST' });
      await new Promise(resolve => setTimeout(resolve, 1000));

      await fetch(`${API_BASE}/?address=${mainAddress}&value=1`, { method: 'POST' });
      await new Promise(resolve => setTimeout(resolve, 2000));
      await fetch(`${API_BASE}/?address=${mainAddress}&value=0`, { method: 'POST' });
  ...
  const StatusIndicator = ({ status }) => (
    <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full border-2 transition-all duration-300 ${
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${status === 1 ? 'bg-white animate-pulse' : 'bg-gray-600'}`}></div>
      </div>
    </div>
  );
  ...
  const MetricCard = ({ title, value, unit, icon: Icon, trend }) => (
    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="text-2xl sm:text-3xl font-bold text-gray-900">
        {value !== null ? `${value} ${unit}` : <div className="h-8 bg-gray-200 rounded animate-pulse w-24"></div>}
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          <MetricCard title="Phase Voltage" value={energyValue} unit="V" icon={Zap} />
          <MetricCard title="Current" value={currentValue} unit="A" icon={Gauge} />
          <MetricCard title="Total KWH" value={totalKwh} unit="kWh" icon={Battery} />
          <MetricCard title="Active Outputs" value={outputStatuses.filter(s => s === 1).length} unit={`of ${outputStatuses.length}`} icon={Power} />
          <MetricCard title="Active Inputs" value={inputStatuses.filter(s => s === 1).length} unit={`of ${inputStatuses.length}`} icon={Activity} />
        </div>
              ...
              <div key={index} className="bg-white border border-gray-200 rounded-lg p-3 text-center shadow-sm hover:shadow-md transition-all hover:scale-105">
                ...
                <div className="flex justify-center mb-2">
                  <div className={`w-6 h-6 rounded-full border-2 transition-all duration-300 ${
                    status === 1 ? 'bg-gradient-to-r from-blue-400 to-blue-600 border-blue-500 shadow-lg shadow-blue-500/50' : 'bg-gray-200 border-gray-300'
                  }`}>
                    <div className={`w-full h-full rounded-full ${status === 1 ? 'bg-white bg-opacity-30 animate-pulse' : ''}`}></div>
                  </div>
                </div>
